...
function Home() {
	...
	useEffect(() => {
		const filteredTrendingProducts = products.filter(item => item.categories === "Wheels");
		const filteredBestSalesProducts = products.filter(item => item.categories === "Lubricant");
		const filteredequipmentProducts = products.filter(item => item.categories === "Equipment");
		const filteredInteriorProducts = products.filter(item => item.categories === "Interior");
		const filteredPopularProducts = products.filter(item => item.categories === "Tyres");
					...
					<Row>
						<Col lg="6" md="6">
							<div className="hero__content">
								...
								<h2>
									{" "}
									Позвольте своему автомобилю качественные детали для обеспечения безопасности на
									дорогах{" "}
								</h2>
								<p>
									Создаём условия для максимальной реализации потенциала Вашего автомобиля.
									Предлагаем широкий ассортимент товаров для автосервиса.
								</p>
					...
					<Row>
						<Col lg="6" md="12" className="count__down-col">
							<div className="clock__top-content">
								<h4 className="text-white fs-6 mb-2">Ограниченное предложение</h4>
