...
function Shop() {
	...
	const handleFilter = (e) =>{
		...
		const filterValue = e.target.value
			...
			if (filterValue === 'Eqipment') {
				const filteredProducts = products.filter((item) => item.categories === 'Eqipment');
		...
		<Helmet title="Shop">
			...
			<section className="pt-0">
				<Container>
					<Row>
						{
						productsData.length === 0? (<h1 className="text-center fs-4">Товары не найдены!</h1>) : (<ProductsList data={productsData}/>)
						}
					</Row>
				</Container>
			</section>
